...
log_file="runs/overnight_logs/epfa_tuned_distill_all.log"
...
ensure_teacher() {
  ...
  local dst="$2"
  if [[ -f "$dst" ]]; then
    log "SKIP teacher copy (${dst} exists)"
    return 0
  fi
  if [[ -f "$src" ]]; then
    cp -f "$src" "$dst"
    log "DONE teacher copy ${dst}"
    return 0
  fi
...
ensure_teacher "runs/deeppcb_baseline/baseline_plain/weights/best.pt" "weights/teacher_deeppcb.pt" || true
ensure_teacher "runs/pku_market_pcb_baseline/baseline_plain/weights/best.pt" "weights/teacher_pku_market_pcb.pt" || true
ensure_teacher "runs/dspcbsd_plus_baseline/baseline_plain/weights/best.pt" "weights/teacher_dspcbsd_plus.pt" || true

run_step "DeepPCB distill_epfa_tuned train" python experiments/distillation/train.py --config configs/train/distillation_deeppcb_epfa_tuned.toml || true
run_step "DeepPCB distill_epfa_tuned eval" python experiments/distillation/evaluate.py --config configs/eval/distillation_deeppcb_epfa_tuned.toml || true
...
log "PIPELINE COMPLETE epfa_tuned_all"
